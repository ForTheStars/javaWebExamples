		...
		<tr>
			<td colspan="7">
				<form action="user_list.action" method="get">
					<s:select theme="simple" list="#ds" listKey="id" listValue="name"
						value="depId" name="depId" headerKey="0" headerValue="全部部门" />
		...
		<s:else>
			<s:iterator value="#pages.datas">
				<tr>
					...
					<td>${nickname }</td>
					<td>${email }</td>
					<td><s:if test="type==0">普通用户</s:if> 
						<s:else>管理员</s:else></td>
