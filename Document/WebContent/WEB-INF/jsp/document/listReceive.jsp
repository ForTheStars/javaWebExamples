		...
		<tr>
			<td colspan="5">
				<form action="document_listReceive.action" method="get">
					<input type="hidden" name="isRead" value="${param.isRead }" /> 关键字:<input
						name="con" value="${con }" /> 选择部门:
					<s:select theme="simple" list="#deps" name="depId" listKey="id"
						listValue="name" headerKey="0" headerValue="选择部门" />
		...
		<s:else>
			<s:iterator value="#pages.datas">
				<tr>
					<td>${id }</td>
					<td><a
						href="document_updateRead.action?id=${id }&isRead=${isRead}">${title}</a></td>
